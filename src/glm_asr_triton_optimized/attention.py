...
def attention_fused_kernel(
    ...
    for start_k in range(0, seq_k, BLOCK_K):
        # Removed early break for IS_CAUSAL as it causes an AST node error in some Triton versions
        # The IS_CAUSAL mask later in the loop will correctly mask out values with -inf.

        curr_k = start_k + offs_k

        # Load K transposed
        k_ptrs = k_ptr + pid_bh * stride_k0 + curr_k[None, :] * stride_k1 + offs_d[:, None] * stride_k2
        k = tl.load(k_ptrs, mask=(curr_k[None, :] < seq_k) & (offs_d[:, None] < head_dim), other=0.0)
        ...
        if HAS_MASK:
            ...
            m_mask = tl.load(m_ptrs, mask=(offs_q[:, None] < seq_q) & (curr_k[None, :] < seq_k), other=float("-inf"))
            qk = qk + m_mask
    ...
    tl.store(out_ptrs, output, mask=(offs_q[:, None] < seq_q) & (offs_d[None, :] < head_dim))
    ...
    if use_triton:
        ...
        if use_fused_attention:
            ...
            if has_mask:
                ...
                stride_m0 = stride_m1 = stride_m2 = 0
                mask_ptr = q_flat
